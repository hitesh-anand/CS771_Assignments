		...
		if len(history) == 0:
			...
			for id in my_words_idx:
				...
				for idx in my_words_idx:
					mask = self.reveal(all_words[idx], curr)
					if mask not in split_dict_test:
						split_dict_test[mask] = []
					split_dict_test[mask].append(idx)
				for item in list(split_dict_test.values()):
					currval+=(len(item)**2)
				if currval < val:
					query = curr
					val = currval
